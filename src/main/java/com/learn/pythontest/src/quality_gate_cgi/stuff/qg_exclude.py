...
'''
Script to find all py and php files in a directory tree and mark them
with the @codingStandardsIgnoreFile header. Use this if you have problems
with third party code from other projects and want to keep it updatable.
...
def _get_py_files(build_folder):
    ...
    @return: a list of py files which have not been marked yet
    '''
    global _py_files
...
def _get_php_files(build_folder):
    ...
    @return: a list of php files which have not been marked yet
    '''
    global _php_files
...
if __name__ == '__main__':
    ...
    for filename in py_files:
        ...
        offset = 0
        while data[offset].startswith('#!'):
            offset += 1
        data.insert(offset, '# @codingStandardsIgnoreFile')
        _write_file(fh, data)
    ...
    for filename in php_files:
        ...
        fh, data = _read_file(filename)
        offset = 0
        while data[offset].startswith('#!') or data[offset].startswith('<?php'):
            offset += 1
        data.insert(offset, '// @codingStandardsIgnoreFile')
        _write_file(fh, data)
