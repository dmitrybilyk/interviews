...
public class ButtonExample implements IsWidget, EntryPoint {
  ...
  public Widget asWidget() {
    if (cp == null) {
      ...
      cp.getBody().addClassName(ThemeStyles.get().style().border());
      ...
      for (Category cat : Category.values()) {
        ...
        btn.addValueChangeHandler(new ValueChangeHandler<Boolean>() {
          ...
          public void onValueChange(ValueChangeEvent<Boolean> event) {
            ...
            } else if (!event.getValue() && btn.getData("cat").equals(Category.TOGGLE)) {
              // when unchecking check item in overflow we manually toggle back to normal
              ToggleButton normal = (ToggleButton) cp.getButtonBar().getWidget(0);
              normal.setValue(true, true, true);
            }
          }
        });
