...
public class ConverterExample implements IsWidget, EntryPoint, Editor<Stock> {
  ...
  /*
   * Simple Converter that allows the DateField and DateCell to display dates/times in a timezone other than the user's
   * own.
   * 
   * This is not meant to be a definitive way of doing this, as there are almost certainly issues here with daylight
   * savings time.
   */
  ...
  private FlowLayoutContainer lc;

  @Override
  public Widget asWidget() {
    if (lc == null) {
      lc = new FlowLayoutContainer();
      ...
      lastTransField.setPropertyEditor(new DateTimePropertyEditor(
          DateTimeFormat.getFormat("EEE, d MMM yyyy HH:mm:ss")));
      ...
      TextButton display = new TextButton("Show", new SelectHandler() {
        ...
        public void onSelect(SelectEvent event) {
          ...
          String dateInLocalTimeZone = s.getLastTrans() == null ? "No value" : s.getLastTrans().toString();
          new MessageBox("User entered date in local timezone:", dateInLocalTimeZone).show();
        }
      });
      lc.add(display);
    }
