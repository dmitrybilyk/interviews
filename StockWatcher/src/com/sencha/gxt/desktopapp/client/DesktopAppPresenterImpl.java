 ...
 * 
 * @see com.sencha.gxt.desktopapp.client.DesktopAppPresenter
 */
public class DesktopAppPresenterImpl implements DesktopAppPresenter {
  ...
  @Override
  public LoginStatus onLogin(LoginModel loginModel) {
    String userName = loginModel.getUserName();
    String password = loginModel.getPassword();
    boolean isNewUser = loginModel.isNewUser();
    if (isExistingUser(userName)) {
      if (isNewUser) {
        return LoginStatus.DUPLICATE_USER_NAME;
      } else if (!validCredentials(userName, password)) {
        return LoginStatus.INVALID_NAME_OR_PASSWORD;
      }
    } else {
      if (isNewUser) {
  ...
  @Override
  public ProfileStatus onUpdateProfile(ProfileModel profileModel) {
